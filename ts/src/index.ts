...
export class SDK extends EventEmitter {
  ...
  constructor({
    ...
    if (doReplicate) {
      swarm.on('connection', (connection: any, peerInfo: any) => {
  ...
  async resolveDNSToKey(domain: string, opts: any = {}) {
    ...
    throw new Error(
      `Unable to resolve DNSLink domain for ${domain}. If you are the site operator, please add a TXT record pointing at _dnslink.${domain} with the value dnslink=/hyper/YOUR_KEY_IN_Z32_HERE`
    );
  }

  // Resolves a string to be a key or opts and resolves DNS
  // Useful for hypercore opts or Hyperdrive
  async resolveNameOrKeyToOpts(nameOrKeyOrURL: string) {
  ...
  async getBee(nameOrKeyOrURL: string, opts: any = {}) {
  ...
  async getDrive(nameOrKeyOrURL: string, opts: any = {}) {
    const coreOpts = {
  ...
  async get(nameOrKeyOrURL: string, opts: any = {}) {
    const coreOpts = {
      ...this.defaultCoreOpts,
      autoJoin: this.autoJoin,
      ...opts
  ...
  makeTopicKey(name: string) {
    const [key] = crypto.namespace(name, 1);
    return key;
  }

  async joinCore(core: any, opts: any = {}) {
  ...
  async close() {
    ...
    await Promise.all([this.corestore.close(), this.swarm.destroy()]);
  }

  replicate(connection: any) {
...
export async function create({
  ...
  const sdkOpts = { ...opts, swarm, corestore };
...
export function throwMissing(name: string) {
  throw new Error(`Missing required argument ${name}`);
}

export function keyToString(key: Uint8Array) {
  return z32.encode(key);
}

export function stringToKey(string: string) {
